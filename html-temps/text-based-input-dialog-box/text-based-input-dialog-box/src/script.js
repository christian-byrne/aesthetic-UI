...
(function($) {
    ...
    $.fn.dialog = function(fn, options) {
        ...
        var cmd = $('<div/>').width(width).appendTo(input_line).cmd({
            prompt: '',
            commands: fn,
        ...
        // for codepen preview
        if (!cmd.isenabled()) {
            cmd.find('.cursor').addClass('blink');
        }
...
var dialog = $('body').dialog(function(input) {
    setTimeout(function() {
        alert(input);
    }, 100);
}, {
...
// temporary fix for initalizing cmd
dialog.cmd().resize(true);